...
osmFilename = joinpath(jemssDir, "data/cities/utrecht/data/roads/osm 2018/utrecht_main_roads_tertiary+_unclassified.osm")
...
v2 = [113, 74, 61, 50, 46, 42] # travel mode 2, classes 1:6
v2 = round.(Int, v2 * 0.86)
v = [
...
cityDir = "$jemssDir/data/cities/utrecht/models/1"
hospitalsFilename = joinpath(cityDir, "hospitals/hospitals_1.csv")
stationsFilename = joinpath(cityDir, "stations/stations_1.csv")
mapFilename = joinpath(cityDir, "maps/map_1.csv")

# shapefile of region border
borderFilename = joinpath(jemssDir, "data/cities/utrecht/data/border/imergis 2017/utrecht_border.shp")
...
for odNode in odNodes nodeChosen[odNode] = true end
...
for field in keys(nodes[1].fields) nodesDeleteField!(nodes, field) end
for field in keys(arcs[1].fields) arcsDeleteField!(arcs, field) end
