...
osmFilename = joinpath(jemssDir, "data/cities/auckland/data/roads/osm 2017/auckland_main_roads_tertiary+.osm")

levels = Set(1:5) # osm classes 1:8 : motorway, trunk, primary, secondary, tertiary, unclassified/residential, service, pedestrian/living street

# arc speed (km/hr) for each travel mode and each arc class
v2 = [87, 45, 45, 45, 45] # travel mode 2, classes 1:5
v2 *= 0.73 # reduce speeds
v = [
...
hospitalsFilename = joinpath(folder, "hospitals/hospitals_1.csv")
stationsFilename = joinpath(folder, "stations/stations_1.csv")
mapFilename = joinpath(folder, "maps/map_1.csv")
