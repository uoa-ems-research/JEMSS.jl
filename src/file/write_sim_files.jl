...
function writeMiscOutputFiles(sim::Simulation)
    ...
    outputFilePath(name::String) = sim.outputFiles[name].path
    if in("ambulances", outputFileKeys)
        writeAmbsFile(outputFilePath("ambulances"), sim.ambulances; writeOutputFields=true)
    end
    if in("calls", outputFileKeys)
        writeCallsFile(outputFilePath("calls"), sim.startTime, sim.calls; writeOutputFields=true)
    end
    if in("hospitals", outputFileKeys)
        writeHospitalsFile(outputFilePath("hospitals"), sim.hospitals; writeOutputFields=true)
    end
end
...
function writeEventToFile!(sim::Simulation, event::Event)
    if !sim.writeOutput || sim.resim.use
        return
    end
...
function writeStatsFiles(sim::Simulation)
    ...
    if in("ambulancesStats", outputFileKeys)
        writeAmbsStatsFile(outputFilePath("ambulancesStats"), stats)
    end
    if in("callsStats", outputFileKeys)
        writeCallsStatsFile(outputFilePath("callsStats"), stats)
    end
    if in("hospitalsStats", outputFileKeys)
        writeHospitalsStatsFile(outputFilePath("hospitalsStats"), stats)
    end
    if in("stationsStats", outputFileKeys)
        writeStationsStatsFile(outputFilePath("stationsStats"), stats)
    end
    if in("statsDict", outputFileKeys)
        writeStatsDictFile(outputFilePath("statsDict"), getPeriodStatsList(sim))
    end
end
...
function writeStatsFiles(sim::Simulation, reps::Vector{Simulation}; periodIndex::Int=nullIndex)
    ...
    if in("ambulancesStats", outputFileKeys)
        writeAmbsStatsFile(outputFilePath("ambulancesStats"), periods)
    end
    if in("callsStats", outputFileKeys)
        writeCallsStatsFile(outputFilePath("callsStats"), periods)
    end
    if in("hospitalsStats", outputFileKeys)
        writeHospitalsStatsFile(outputFilePath("hospitalsStats"), periods)
    end
    if in("stationsStats", outputFileKeys)
        writeStationsStatsFile(outputFilePath("stationsStats"), periods)
    end
    if in("statsDict", outputFileKeys)
        writeStatsDictFile(outputFilePath("statsDict"), periods)
    end
end
