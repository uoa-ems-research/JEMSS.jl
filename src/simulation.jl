...
################
	...
	elseif eventForm == considerDispatch
		...
		if sim.resim.use
			ambIndex = resimNearestFreeAmbToCall(sim, call)
		else
			ambIndex = nearestFreeAmbToCall!(sim, call)
		end
		# if no ambulance found, add call to queue,
		# will respond when an ambulance becomes idle
		if ambIndex == nullIndex
			addCallToQueue!(queuedCallList, call)
...
function nearestFreeAmbToCall!(sim::Simulation, call::Call)
